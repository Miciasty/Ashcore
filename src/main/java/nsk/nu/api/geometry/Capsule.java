...
public record Capsule(Vector3 a, Vector3 b, double radius) {
    ...
    public double distanceSqTo(Vector3 p){
        ...
        double t = Math.max(0, Math.min(1, ap.dot(ab) / ab.dot(ab)));
        Vector3 closest = a.add(ab.mul(t));
        double d2 = p.sub(closest).dot(p.sub(closest)) - radius*radius;
        return Math.max(0.0, d2);
    }

    /** Ray vs capsule; returns t or +INF if no hit. */
    public double rayIntersectT(nsk.nu.api.geometry.Ray ray){
        ...
        if (Math.abs(aTerm) < 1e-12) return Double.POSITIVE_INFINITY;
