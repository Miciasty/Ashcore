...
public final class AshcoreQuickStart {
    public static void main(String[] args) {
        ...
        {id: 'run', title: 'Compile and run', html: '<p>Run these commands in the consumer directory. The first command also copies runtime dependencies into <code>target/dependency</code>.</p>'+code('shell','Windows PowerShell',`mvn compile dependency:copy-dependencies -DincludeScope=runtime
java -cp "target/classes;target/dependency/*" AshcoreQuickStart`)+ '<p>On Linux or macOS, use a colon in the classpath:</p>'+code('shell','Linux / macOS',`java -cp "target/classes:target/dependency/*" AshcoreQuickStart`)+code('text','Expected output',`distance=2.0, hitX=0.0
