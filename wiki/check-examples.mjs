...
for (const example of examples) {
  const output = run('java',['-ea','-cp',classes,example.name]);
  console.log(`PASS ${example.page}: ${example.name}${output ? `\n${output}` : ''}`);
}
console.log(`Compiled Ashcore source and ran ${examples.length} documentation examples with assertions enabled.`);
